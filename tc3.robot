...
Test case 3
    ...
    ${dateOfBirth}     Set Variable    ${ws.cell(4,2).value}
    Log to console  dateOfBirth=${dateOfBirth}

    ${height}     Set Variable    ${ws.cell(4,3).value}
    Log to console  height=${height}

    ${weight}   Set Variable    ${ws.cell(4,4).value}
    Log to console  weight=${weight}

    ${fname}   Set Variable    ${ws.cell(4,5).value}
    Log to console  fname=${fname}

    ${lname}   Set Variable    ${ws.cell(4,6).value}
    Log to console  lname=${lname}

    ${idCard}   Set Variable    ${ws.cell(4,7).value}
    Log to console  idCard={$idCard}
    
    ${laserIdCard}   Set Variable    ${ws.cell(4,8).value}
...
# Step 1/2
    ...
    Click Element   //*[@id="__layout"]/div/div[1]/div[3]/div[1]/div/div/div/div[3]/div[1]
    SLEEP   5
