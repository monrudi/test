...
Test case 2
    ...
    ${dateOfBirth}     Set Variable    ${ws.cell(3,2).value}
    Log to console  dateOfBirth=${dateOfBirth}

    ${height}     Set Variable    ${ws.cell(3,3).value}
    Log to console  height=${height}

    ${weight}   Set Variable    ${ws.cell(3,4).value}
    Log to console  weight=${weight}

    ${fname}   Set Variable    ${ws.cell(3,5).value}
    Log to console  fname=${fname}

    ${lname}   Set Variable    ${ws.cell(3,6).value}
    Log to console  lname=${lname}

    ${idCard}   Set Variable    ${ws.cell(3,7).value}
    Log to console  idCard={$idCard}
    
    ${laserIdCard}   Set Variable    ${ws.cell(3,8).value}
...
# Step 1/2
    Click Element   //*[@id="__layout"]/div/div[1]/div[2]/div/form/div[1]/div/div[1]/div/div/label[1]/span
    ...
    Click Element   //*[@id="__layout"]/div/div[1]/div[3]/div[1]/div/div/div/div[2]/div[2]/span[1]
    SLEEP   5
...
# Next to 2/3
    ...
    Click Element   xpath: //*[contains(text(), "นาย")]	
